...
import { AdminAccountSchema } from "./schema";

export type AdminAccountFieldType = z.infer<typeof AdminAccountSchema>;
type PropsType = {
  formValue: AdminAccountFieldType;
};

const AdminManagementForm = ({ formValue }: PropsType) => {
  const id = formValue?.id;
  const fieldSchema = id
    ? AdminAccountSchema.refine(
    ...
    : AdminAccountSchema.extend(profileSchema).refine(
        (data) => data.newPassword === data.confirmPassword,
        {
          message: "Passwords do not match",
          path: ["confirmPassword"],
